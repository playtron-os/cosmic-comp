...
use crate::{
    ...
    wayland::protocols::{layer_surface_dismiss::was_dismiss_armed, workspace::WorkspaceHandle},
};
...
fn layer_surfaces<'a>(
    ...
    layers.into_iter().filter_map(move |(s, loc)| {
        ...
        // ever armed to dismiss on a click elsewhere is a popover, a menu, the
        // launcher — and still one while it animates out after that click.
        if element_filter == ElementFilter::ExcludeTransientShell
            && (was_dismiss_armed(s.wl_surface()) || s.namespace() == WORKSPACE_OVERVIEW_NAMESPACE)
        {
            return None;
        }
